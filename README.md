...
#SBATCH --array=1-353%25 #runs 25 percent of genes at a time      
#SBATCH --ntasks=1
...
#SBATCH --partition=short
#SBATCH --array=1-number of genes remaining after TAPER%25 #runs 25 percent of genes at a time
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=2
#SBATCH --mem=2G
#SBATCH --mail-user=*email*@kew.org
...
#SBATCH --partition=short
#SBATCH --array=1-*number of genes remaining*%25 #runs 25 percent of genes at a time
#SBATCH --ntasks=1
...
bash PASTA-taster-2.sh
```

Upload script
```
scp OptrimAl-script-2.sh gflanaga@gruffalo.cropdiversity.ac.uk:/home/gflanaga/scratch/private/test/TMP_Irsalina_Syzygium/HP_out/alignments/edited/alM_r_o
```
### PASTA taster
...
while read cutoff_trim
do
        cd /home/DIR/HP_out/alignments/edited/alM_r_o
        mkdir $cutoff_trim

        for alignment in *_cT.fasta
        do
          /home/DIR/apps/trimAl/source/trimal -in ${alignment} -out ${cutoff_trim}/${alignment/.fasta}_f.fasta -htmlout ${cutoff_trim}/${alignment/.fasta}_f.html -gt $cutoff_trim

                # check if alignment was trimmed to extinction by trimAl

                if grep ' 0 bp' ${cutoff_trim}/${alignment/.fasta}_f.fasta
                then
                        rm -f ${cutoff_trim}/${alignment/.fasta}_f.fasta
                fi
        done

        cd /home/DIR/HP_out/alignments/edited/alM_r_o/${cutoff_trim}
        python3 /home/gflanaga/scratch/apps/AMAS-master/amas/AMAS.py summary -f fasta -d dna -i *_f.fasta

        mv summary.txt ../summary_${cutoff_trim}.txt

done < /home/gflanaga/scratch/private/test/TMP_Irsalina_Syzygium/HP_out/alignments/edited/alM_r_o/cutoff_trim.txt

Rscript /home/gflanaga/scratch/private/test/TMP_Irsalina_Syzygium/HP_out/alignments/edited/alM_r_o/optrimal-2.r
```

Uploading PASTA taster
```
scp optrimal-2.r DIR/HP_out/alignments/edited/alM_r_o
...
setwd("/home/DIR/HP_out/alignments/edited/alM_r_o")
cutoff_trim <- readLines('cutoff_trim.txt')
...
Upload cutoff txt (same as before
```
scp cutoff_trim.txt DIR/HP_out/alignments/edited/alM_r_o
```

Run OptrimAl
```
sbatch /home/DIR/HP_out/alignments/edited/alM_r_o/OptrimAl-script-2.sh

```

Run R if it doesn't go through
```
sbatch /home/DIR/HP_out/alignments/edited/alM_r_o/optrimal.R
```
